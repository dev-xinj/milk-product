...
public class ItemProductController {
    ...
    @GetMapping()
    public ResponseEntity<ResponseObject> getListItemProduct(@RequestParam int pageNo, @RequestParam int pageSize) {
        return ResponseEntity.status(HttpStatus.OK).body(new ResponseObject(HttpStatus.OK.name(), "Successfully",
                itemProductService.getAllItemProduct(pageNo, pageSize)));
    }
    ...
    @PutMapping("{id}")
    public ResponseEntity<ResponseObject> updateItemProduct(@PathVariable Long id, @RequestBody ItemProductDTO itemProductDTO) {
        return ResponseEntity.status(HttpStatus.OK).body(new ResponseObject(HttpStatus.OK.name(), "Save Successfully",
